...
def play(game):
    ...
    logger.info(f"Get next free cube at: {cube}")
    if cube:
        game.board.update_cube_numbers(cube)
        logger.info(f"Update opt and none for {cube}")

        if len(cube.optional_nums) == 0:  # free cube found but no more optionals, go back to iterate last number
            logger.info(f"No more opt for {cube}")
            logger.info(f"we ran into a wall!...that's a shame, we'll go back and start over")
            return False
        else:
            for inum in iter(cube.optional_nums):
                cube.number = inum
                logger.info(f"Cube: {cube}, number as been set to {cube.number}")
                if not game.board.set_cube_in_board(cube):
                    continue
                print(game.board.draw_board())
                if play(game):
                    logger.info(f"Lucky guess! {cube}")
                    return True
                else:
                    cube.number = None
            return False
...
def main():
    ...
    b.set_numbers_in_cubes(game_level)
